...
pub fn query_os_address(deps: Deps, os_id: u32) -> StdResult<Binary> {
    ...
    match os_address {
        Err(_) => return Err(StdError::generic_err(VersionError::MissingOsId { id: os_id }.to_string())),
...
pub fn query_code_id(deps: Deps, module: String, version: String) -> StdResult<Binary> {
    ...
    match code_id {
        Err(_) => return Err(StdError::generic_err(VersionError::MissingCodeId { module, version }.to_string())),
