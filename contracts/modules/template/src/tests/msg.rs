...
#[rstest]
pub fn test_successfully_update_config_msg_with_proxy_address(deps: (MockDeps, Env)) {
    let (mut deps, env) = deps;
    let msg = ExecuteMsg::Base(BaseExecuteMsg::UpdateConfig {
        proxy_address: Some("new_proxy_address".to_string()),
    });
...
pub fn test_successfully_update_traders_add_many(deps: (MockDeps, Env)) {
    ...
    assert_eq!(
        &state,
        &BaseState {
            proxy_address: Addr::unchecked(TREASURY_CONTRACT.to_string()),
...
fn assert_equal_base_state(
    ...
    assert_eq!(
        actual_state,
        &BaseState {
            proxy_address: Addr::unchecked(expected_proxy.to_string()),
            traders: expected_traders
                .into_iter()
                .map(|t| Addr::unchecked(t))
                .collect(),
