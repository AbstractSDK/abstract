...
use abstract_os::vault::{DepositHookMsg, ExecuteMsg};
...
fn deposit_and_withdraw_with_native_token() {
    ...
    // Add UST to proxy through vault dapp contract interaction
    app.execute_contract(
        sender.clone(),
        vault_dapp.clone(),
        &ExecuteMsg::ProvideLiquidity {
    ...
    assert_eq!(10_000_000u128, proxy_res.value.u128());

    // First addition to pool so we own it all -> 10 UST
    let owned_locked_value =
        liquidity_token_value(&app, &vault_l_token, &base_contracts.proxy, &sender);
...
fn liquidity_token_value(app: &App, l_token: &Addr, proxy_addr: &Addr, owner: &Addr) -> Uint128 {
    ...
    let vault_res: TreasuryMsg::TotalValueResponse = app
        .wrap()
        .query_wasm_smart(proxy_addr, &TreasuryMsg::QueryMsg::TotalValue {})
        .unwrap();
