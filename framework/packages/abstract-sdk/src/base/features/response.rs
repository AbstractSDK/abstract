...
use cosmwasm_std::{
    wasm_execute, Attribute, Binary, CosmosMsg, Deps, Event, ReplyOn, Response, SubMsg,
};
...
pub trait ExecutionStack: Sized + AccountIdentification {
    ...
    fn _unwrap_for_response(&mut self, deps: Deps) -> AbstractSdkResult<Vec<SubMsg>> {
        let proxy_addr = self.proxy_address(deps)?.to_string();
...
pub trait ResponseGenerator: ExecutionStack + CustomEvents + CustomData {
    fn _generate_response(&mut self, deps: Deps) -> AbstractSdkResult<Response> {
        let resp = Response::new()
            .add_events(self.events())
            .add_attributes(self.attributes())
            .add_submessages(self._unwrap_for_response(deps)?);
