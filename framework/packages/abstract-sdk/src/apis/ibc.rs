...
pub struct MiddlewareForwardMemoBuilder {
    ...
    retries: Option<u8>,
    next: Option<serde_cw_value::Value>,
}
...
impl MiddlewareForwardMemoBuilder {
    ...
    /// Add next hop to middleware
    pub fn next(mut self, forward_memo: MiddlewareForwardMemoBuilder) -> Self {
        self.next = Some(forward_memo.build_value());
        self
    }
    ...
    fn build_value(self) -> serde_cw_value::Value {
        ...
        if let Some(timeout) = timeout {
            forward_value.push((Value::String("timeout".to_owned()), Value::String(timeout)))
        }
        if let Some(retries) = retries {
            forward_value.push((Value::String("retries".to_owned()), Value::U8(retries)))
        }
        if let Some(next) = next {
            forward_value.push((Value::String("next".to_owned()), next))
        }
