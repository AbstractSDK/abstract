...
pub trait ApiQuery<S: ModuleIdentification + DepsAccess>:
    AbstractApi<S> + ApiIdentification
{
    fn smart_query<T: DeserializeOwned>(
        ...
        let querier = self.base().deps().querier;
        querier
...
where
    S: ModuleIdentification + DepsAccess,
