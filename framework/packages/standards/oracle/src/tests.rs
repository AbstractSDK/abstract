...
use crate::{Fee, FeeOnInput, OracleCommand, OracleError, Return, Spread};
...
impl DexCommandTester {
    pub fn new<T: OracleCommand + 'static>(chain: ChainData, adapter: T) -> Self {
        Self {
            chain,
    ...
    ) -> Result<Vec<CosmosMsg>, OracleError> {
    ...
    pub fn test_simulate_swap(
        ...
        offer_asset: Asset,
        ask_asset: AssetInfo,
    ) -> Result<(Return, Spread, Fee, FeeOnInput), OracleError> {
        let deps = mock_dependencies(self.chain.clone());
        let result = self
            .adapter
