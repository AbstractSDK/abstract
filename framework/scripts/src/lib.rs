use cw_orch::{prelude::{networks::{JUNO_1, UNI_6, PHOENIX_1, PISCO_1, PION_1, OSMO_5}, *}, daemon::ChainInfo};

const GAS_TO_DEPLOY: u64 = 60_000_000;
const SUPPORTED_CHAINS: &[ChainInfo] = &[JUNO_1, UNI_6, PHOENIX_1, PISCO_1, PION_1, NEUTRON_1, OSMO_5];
...
pub async fn assert_wallet_balance(mut chains: &[ChainInfo<'_>]) {
    ...
    for chain_info in chains{
        let chain = DaemonAsyncBuilder::default().chain(chain_info.clone()).build().await.unwrap();
        ...
        let balance = bank.balance(chain.sender(),Some(fee_token.denom.clone())).await.unwrap()[0].clone();

        log::debug!("Checking balance {} on chain {}, address {}. Expecting {}{}", balance.amount,chain_info.chain_id,chain.sender(), fee, fee_token.denom.as_str());
