...
Abstract provides multiple module bases, as detailed in our section on [modules](../framework/module_types.md).
...
In this tutorial we will be working on an [`App` module](../framework/module_types.md#apps).
...
The Module identifier (Module ID) is a string that will identify your application. We covered it in detail in the section on modules, [here](../framework/module_types.md#module-ids).
...
3. `metadata`: An optional URL that can be used to retrieve data off-chain. Can be used with the Abstract Metadata Standard to automatically generate interactive front-end components for the module. This is explained in more detail in the [metadata section](metadata.md).
...
A special feature of these functions is that we insert the instance of your module into the function's attributes. This enables you to access the module struct in your code. You will learn why this is such a powerful feature in the next section on the [Abstract SDK](./sdk.md).
...
In the case of adapters, the handlers are the same, except for `with_migrate` and `with_sudo` that are missing for reasons we explain in the [adapter section](../framework/module_types.md#adapters).
...
Below, we examine each handler in greater detail. The `base` fields and variants mentioned in the messages below are defined by [the base module type](../framework/module_types.md) that you chose to use, an `App` in this case.
...
the [dependencies section](./dependencies.md).
