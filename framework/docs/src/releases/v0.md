...
- Abstract standalone package

### Changed

- Breaking: module factory will try to fill `base` for standalone modules if it's present(for example `"base": null`)
- Manager will try to check dependencies on standalones
