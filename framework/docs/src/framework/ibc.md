...
```admonish info
The account creation process is covered in detail in the [Account Factory](../platform/account_factory.md) section.
```
...
```admonish info
Need a refresher on adapters? Check out the [Adapters](../framework/module_types.md#adapters) section.
```
