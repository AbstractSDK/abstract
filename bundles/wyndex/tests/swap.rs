...
mod staking {
    ...
    fn create_pair_and_distributions() {
        ...
        suite
            .distribute_funds(pair_info.staking_addr.clone(), user.as_str(), &[coin(100, ujuno)])
            .unwrap();
        suite
            .distribute_funds(pair_info.staking_addr.clone(), user.as_str(), &[coin(100, uluna)])
            .unwrap();
        suite
            .distribute_funds(pair_info.staking_addr.clone(), user.as_str(), &[coin(100, test)])
            .unwrap();
        suite
