...
use abstract_core::{
    ans_host::ExecuteMsgFns as AnsHostExecuteMsgFns, manager::QueryMsgFns,
    objects::gov_type::GovernanceDetails, version_control::ExecuteMsgFns as VcExecMsgFns,
};
...
use abstract_gas_station_app::{
    ...
    GasStationApp, GasStationQueryMsgFns,
};
...
impl GasStationTest<OsmosisTestTube> {
    fn setup(initial_balance: Option<Vec<Coin>>) -> anyhow::Result<Self> {
        ...
        account.install_app(&gas_station, &GasStationInstantiateMsg {}, None)?;
...
fn deploy_gas_station<Env: CwEnv>(tube: &Env) -> GasStationApp<Env> {
    ...
    station
        .deploy(VERSION.parse().unwrap(), DeployStrategy::Try)
        .unwrap();
...
// TODO: It's not allowed on osmosis
// #[test]
