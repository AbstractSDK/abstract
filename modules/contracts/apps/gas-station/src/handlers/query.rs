use cosmwasm_std::{to_json_binary, Binary, Deps, Env, Order, StdError};
...
pub fn query_handler(
    ...
    match msg {
        GasStationQueryMsg::GradeInfo { grade } => to_json_binary(&query_grade(deps, app, grade)?),
        GasStationQueryMsg::GradeList {} => to_json_binary(&query_grade_list(deps, app)?),
        GasStationQueryMsg::GasPassHolders { grade } => {
            to_json_binary(&query_gas_pass_holders(deps, app, grade)?)
        }
        GasStationQueryMsg::GasPass { holder } => {
            to_json_binary(&query_gas_pass(deps, app, holder)?)
        }
    }
    .map_err(Into::into)
}
