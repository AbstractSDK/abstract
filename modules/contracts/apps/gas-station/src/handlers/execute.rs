...
use cosmwasm_std::{Addr, Coin, CosmosMsg, DepsMut, Env, MessageInfo, Response, Timestamp};
...
pub fn execute_handler(
    ...
    match msg {
        ...
        GasStationExecuteMsg::ActivateGasPass { grade, recipient, expiration, bypass_pass_check } => {
            activate_gas_pass(deps, env, info, app, grade, recipient, expiration, bypass_pass_check)
        }
...
fn create_grade(
    ...
    GRADES.update(deps.storage, grade.clone(), |x| -> GasStationResult<Grade> {
        match x {
...
fn activate_gas_pass(
    ...
    let allowance_msg = app.fee_granter(deps.as_ref(), None).grant_basic_allowance(&recipient, pump.fuel_mix.clone(), expiration)?;
    let allowance_msg = app.executor(deps.as_ref()).execute(vec![allowance_msg.into()])?;

    // let allowance = BasicAllowance {
    //     spend_limit: pump.fuel_mix.iter().map(|coin| SdkCoin {
    ...
    //     granter: _env.contract.address.to_string(),
    ...
    // };
    //
    // let allowance_msg = CosmosMsg::Stargate {
    ...
    GRADE_TO_USERS.update(deps.storage, &grade, |x| -> GasStationResult<HashSet<Addr>> {
        match x {
            Some(mut set) => {
...
fn deactivate_gas_pass(
    ...
    let allowance_msg = app.fee_granter(deps.as_ref(), None).revoke_allowance(&holder)?;
    let allowance_msg = app.executor(deps.as_ref()).execute(vec![allowance_msg.into()])?;
    ...
    if let Some(pass) = maybe_grade {
        ...
        GRADE_TO_USERS.update(deps.storage, &pass.grade, |x| -> GasStationResult<HashSet<Addr>> {
            match x {
                Some(mut set) => {
                    set.remove(&holder);
                    Ok(set)
    ...
    Ok(app.custom_tag_response(Response::new().add_message(allowance_msg), "dispense_gas", vec![("recipient", holder.as_str())]))
}
