use cosmwasm_std::{DepsMut, ensure_eq, Env, Reply, Response};
...
pub fn create_denom_reply(deps: DepsMut,
                          env: Env,
                          app: GasStationApp,
                          reply: Reply) -> AppResult {
    ...
    ensure_eq!(pump.denom, new_token_denom, AppError::PendingGasPumpDoesNotMatchCreatedGasPump {
        pending: grade.clone(),
    ...
    let before_send_hook = app.executor(deps.as_ref()).execute(vec![before_send_hook])?;

    GAS_PUMPS.save(deps.storage, grade.clone(), &pump)?;

    Ok(app.custom_tag_response(Response::new().add_message(before_send_hook)
                               , "create_denom", vec![("grade", "grade")]))
}
