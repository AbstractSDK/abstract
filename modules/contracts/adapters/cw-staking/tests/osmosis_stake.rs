...
use abstract_cw_staking::{
    ...
    msg::{
        ExecuteMsg, InstantiateMsg, QueryMsg, StakingAction, StakingExecuteMsg, StakingQueryMsgFns,
    },
};
...
fn claim_rewards_fails() -> anyhow::Result<()> {
    ...
    if let AbstractInterfaceError::Orch(CwOrchError::StdErr(e)) = err {
        let expected_err =
            CwStakingError::NotImplemented("osmosis does not support claiming rewards".to_owned());
