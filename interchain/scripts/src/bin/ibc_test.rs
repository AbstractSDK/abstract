...
use abstract_std::{ibc_client, proxy, IBC_CLIENT, IBC_HOST, MANAGER, PROXY};
...
fn ibc_test(
    ...
    // let manager = Manager::new(MANAGER, src_daemon.clone());
    // manager.upload()?;

    // src_abstract.version_control.propose_modules(vec![
    //     (
    //         ModuleInfo::from_id(MANAGER, "0.22.1".into())?,
    //         ModuleReference::AccountBase(manager.code_id()?),
    //     ),
    //     (
    //         ModuleInfo::from_id(IBC_HOST, "0.22.1".into())?,
    //         ModuleReference::Native(src_abstract.ibc.host.address()?),
    //     ),
    // ])?;
