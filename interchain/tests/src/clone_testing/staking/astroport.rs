...
impl AstroportStake {
    ...
    fn new(chain: CloneTesting) -> anyhow::Result<Self> {
        let lp_token_addr = Addr::unchecked(LP_TOKEN_ADDR);
        let minter_response: cw20::MinterResponse = chain
            .wasm_querier()
            .smart_query(&lp_token_addr, &cw20::Cw20QueryMsg::Minter {})?;
        Ok(Self {
            chain,
            lp_asset: (
                ASSET_ENTRY_LP.to_owned(),
                AssetInfoUnchecked::cw20(lp_token_addr.to_string()),
            ),
            minter: Addr::unchecked(minter_response.minter),
        })
    }
    // TODO: fix if possible, tests would be more stable using our assets
    // fn new(chain: CloneTesting) -> anyhow::Result<Self> {
