...
pub fn test_send_funds() -> AnyResult<()> {
    ...
    let (origin_account, remote_account_id) =
        create_test_remote_account(&abstr_juno, JUNO, STARGAZE, &interchain, vec![])?;
    ...
    let remote_account = AccountI::load_from(&abstr_stargaze, remote_account_id.clone())?;
    let client = remote_account.config()?;

    log::info!("client address {:?}", client);
    ...
    let balance = stargaze
        .bank_querier()
        .balance(remote_account_config.addr(), None)?;
